    ...
    func get(_ request: Request) throws -> Future<ServerResponse> {
        ...
        return request.globalAsync { promise in
            ...
            if authentication.permission.isAdmin == false {
                ...
                devices = try Device.collection.find(Query(aqt: AQT.in(key: "hostDeviceId", in: Array(hostDeviceIds))) && Query(aqt: AQT.in(key: "type", in: Device.DeviceType.temperatureSensorValues)), projecting: [
                    "_id", "name"
                ])
                ...
                guard deviceIds.count > 0 else {
                    ...
                    return promise.submit(try request.renderEncoded("temperatures", context))
                }
            } else {
                devices = try Device.collection.find(Query(aqt: AQT.in(key: "type", in: Device.DeviceType.temperatureSensorValues)), projecting: [
                    "_id", "name"
                ])
                if let deviceId = filteredDeviceId {
                    deviceIds.insert(deviceId)
            ...
            let query = try "SELECT mean(\"temperature\") AS \"temperature\" FROM \"brew\".\"autogen\".\"temperature\" WHERE \((filteredDeviceId == nil ? "time > now() - 15m" : "time > now() - 30m"))\(deviceFilter) GROUP BY \"deviceId\" FILL(none)".urlEncodedFormEncoded()
            requestClient.get("\(influxdbUrl)/query", headers: headers, beforeSend: { request in
                ...
                do {
                    ...
                    if let data = response.http.body.data, let dictionary = try JSONSerialization.jsonObject(with: data, options: JSONSerialization.ReadingOptions(rawValue: 0)) as? [String: Any], let result = (dictionary["results"] as? [[String: Any]])?.first {
                        var allValues: [(deviceId: ObjectId, value: Double, date: Date)] = []
                        if let series = result["series"] as? [[String: Any]] {
                            for group in series {
                                ...
                                for value in values {
                                    guard let dateString = value.first as? String, let date = Formatter.iso8601Nano.date(from: dateString), let itemValue = value.last as? Double else { continue }
                        ...
                        for value in allValues {
                            ...
                            let string = "<tr><td>\(deviceName)</td><td>\(value.date.longString)</td><td>\(Double.temperatureFromDouble(value.value))</td></tr>"
