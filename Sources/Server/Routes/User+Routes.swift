...
struct UserRouter {
    ...
    func post(_ request: Request) throws -> Future<ServerResponse> {
        return request.globalAsync { promise in
            ...
            do {
                try User.register(credentials: EmailPassword(email: formData.email, password: formData.password), permission: (formData.admin ?? false ? .admin : .regular))
                return promise.succeed(result: request.serverStatusRedirect(status: .ok, to: "/users"))
            } catch {
                return promise.succeed(result: request.serverStatusRedirect(status: .conflict, to: "/users?emailTaken=true"))
            }
        }
    }
    ...
    func getUser(_ request: Request) throws -> Future<ServerResponse> {
        return request.globalAsync { promise in
            ...
            guard authentication.permission.isAdmin || userId == authentication.userId else {
                return promise.succeed(result: request.serverStatusRedirect(status: .forbidden, to: "/users"))
            }
            guard let document = try User.collection.findOne("_id" == userId, projecting: [
                "password": false,
                "totpToken": false
            ]) else {
            ...
            if request.jsonResponse {
                ...
                let link = "/users/\(userId.hexString)?"
                ...
                let context = TemplateData.dictionary([
                    ...
                    (userPermission == .admin ? "adminEnabled" : "adminDisabled"): .string("checked"),
    ...
    func logout(_ request: Request) throws -> Future<ServerResponse> {
        return request.globalAsync { promise in
            ...
            if request.jsonResponse {
                ...
                let response = request.redirect(to: "/users/login")
                response.http.cookies = User.logoutCookie
                return promise.succeed(result: ServerResponse.response(response))
            }
        }
    ...
    func postUser(_ request: Request) throws -> Future<ServerResponse> {
        return request.globalAsync { promise in
            ...
            if formData.action == "delete" {
                if let otherUser = try User.collection.findOne("_id" != userId && "permission" == 2), let otherUserId = otherUser.objectId {
                    try BrewContainer.collection.update("userId" == userId, to: ["$set": ["userId": otherUserId]], multiple: true)
                    try HostDevice.collection.update("userId" == userId, to: ["$set": ["userId": otherUserId]], multiple: true)
        ...
        return request.globalAsync { promise in
            ...
            if request.jsonResponse {
                ...
                for document in documents {
                    guard let email = document["email"] as? String, let id = document.objectId, let permission = try? document.extractUserPermission("permission") else {
                        continue
                    }
                    let totpActivated = document["totpActivated"] as? Bool ?? false
                    let badge = (totpActivated ? "success" : "danger")
                    let string = "<tr onclick=\"location.href='/users/\(id.hexString)'\"><td>\(email)</td><td><span class=\"badge badge-\(badge)\">\((totpActivated ? "Enabled" : "Disabled"))</span></td><td>\(permission.description)</td></tr>"
                    tableData.append(string)
                }
                let context = TemplateData.dictionary([
                    "tableData": .string(tableData),
    ...
    func postLogin(_ request: Request) throws -> Future<ServerResponse> {
        return request.globalAsync { promise in
            ...
            let response = request.redirect(to: (formData.getReferrer == "none" ? "/containers" : formData.getReferrer))
    ...
    func postUserTotp(_ request: Request) throws -> Future<ServerResponse> {
        return request.globalAsync { promise in
            ...
            if action == "deactivate" {
                ...
                return promise.succeed(result: request.serverStatusRedirect(status: .ok, to: "/users/\(userId.hexString)"))
