...
struct OAuthRouter {
    ...
    func getAuthorize(_ request: Request) throws -> Future<ServerResponse> {
        return request.globalAsync { promise in
            // For App Review
//            return promise.succeed(result: request.serverRedirect(to: "reaumur://oauth?code=\(try User.login(credentials: EmailPassword(email: "demo@blu.bz", password: "Demo123456")).authorizationCode(redirectUri: "reaumur://oauth", clientId: ObjectId("59dfa6eb03088d0006295717"), scope: "user", state: nil))"))
            ...
            guard let client = try BrewClient.collection.findOne("_id" == formData.client_id) else {
    ...
    func postAccessToken(_ request: Request) throws -> Future<ServerResponse> {
        return request.globalAsync { promise in
            ...
            // For App Review
//            let clientId = try ObjectId("59dfa6eb03088d0006295717")
//            let clientSecretString = "BVcZElBiznG28fNDdzG7vepDGnDeniki3yBBi2Dk"
            guard let client = try BrewClient.collection.findOne("_id" == clientId) else {
