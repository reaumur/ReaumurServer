...
extension URL {
//    func log() {
//        Logger.info("--URL: \(self)--")
//        Logger.info("Host: \(host)")
//        Logger.info("Absolute: \(absoluteURL)")
//        Logger.info("Base: \(baseURL)")
//        Logger.info("Port: \(port)")
//        Logger.info("Scheme: \(scheme)")
//        Logger.info("Standardized: \(standardized)")
//        Logger.info("Absolute Host: \(absoluteURL.host)")
//        Logger.info("File: \(isFileURL)")
//    }
...
extension Double {
    ...
    static let precentageNumberFormatter: NumberFormatter = {
        let precentageNumberFormatter = NumberFormatter()
        precentageNumberFormatter.numberStyle = .percent
        precentageNumberFormatter.minimumFractionDigits = 0
        precentageNumberFormatter.maximumFractionDigits = 1
        return precentageNumberFormatter
    }()
    
    /// Converts a temperature `Double` to a `String`.
    ///
    /// - Parameter double: The `Double` temperature to convert.
    /// - Returns: The `String` result
    static func temperatureFromDouble(_ double: Double?) -> String {
        if let double = double {
            if let string = Double.numberFormatter.string(for: double) {
                return "\(string)°"
            } else {
                return "0°"
            }
    ...
    /// Converts a humidity `Double` to a `String`.
    ///
    /// - Parameter double: The `Double` temperature to convert.
    /// - Returns: The `String` result.
    static func humidityFromDouble(_ double: Double?) -> String {
        if let double = double {
            if let string = Double.precentageNumberFormatter.string(for: (double / 100.0)) {
                return string
            } else {
                return "0%"
            }
...
extension Document {
    ...
    func extractHostDevicePin(_ key: String) throws -> Device.HostDevicePin {
        let value = try extractInteger(key)
        guard let devicePin = Device.HostDevicePin(rawValue: value) else {
            throw ServerAbort(.notFound, reason: "Invalid update led pin")
        }
        return devicePin
    }
    func extractDeviceType(_ key: String) throws -> Device.DeviceType {
        let value = try extractInteger(key)
        guard let deviceType = Device.DeviceType(rawValue: value) else {
            throw ServerAbort(.notFound, reason: "Invalid device type")
        }
        return deviceType
    }
    func extractTemperature(_ key: String) -> Double {
        guard let value = self[key]?.doubleValue else {
            return -100
        }
        return value
    }
    func extractTriggerTemperature(_ key: String) -> Double {
        guard let value = self[key]?.doubleValue else {
            return 0
        }
        return value
    ...
    func extractLogType(_ key: String) throws -> Log.LogType {
        let value = try extractInteger(key)
        guard let logType = Log.LogType(rawValue: value) else {
            throw ServerAbort(.notFound, reason: "Invalid log type")
        }
        return logType
    }
}
...
extension Formatter {
    ...
    static let iso8601Nano: DateFormatter = {
        let formatter = DateFormatter()
        formatter.calendar = Calendar(identifier: .gregorian)
        formatter.locale = Locale(identifier: "en_US_POSIX")
        formatter.timeZone = TimeZone(secondsFromGMT: 0)
